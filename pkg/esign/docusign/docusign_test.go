...
func TestDocusign(t *testing.T) {
	...
	if true {
		env := &model.EnvelopeDefinition{
			EmailSubject: "[Go eSignagure SDK] - Please sign this doc(embeded)",
			EmailBlurb:   "Please sign this test document",
			Status:       "sent",
			Documents: []model.Document{
				{
		...
		summary, err := docu.SendEnvelope(context.TODO(), env, &esign.UploadFile{
			ContentType: "application/pdf",
			FileName:    "invitation letter.pdf",
			ID:          "1",
			Reader:      f1,
		...
		assert.True(t, err == nil && summary.EnvelopeID != "")

		// follow  https://developers.docusign.com/esign-rest-api/guides/concepts/embedding#step-2-generate-the-recipient-signing-url
